local c = require('onedarkbleak.colors')
local cfg = vim.g.onedarkbleak_config
local util = require("onedarkbleak.util")
...
hl.common = {
    ...
    SpellRare = {fg = c.none, fmt = "undercurl", sp = c.blue},
...
if vim.api.nvim_call_function("has", { "nvim-0.8" }) == 1 then
    hl.treesitter = {
        ...
        ["@keyword"] = {fg = c.blue, fmt = cfg.code_style.keywords},
        ["@keyword.conditional"] = {fg = c.blue, fmt = cfg.code_style.keywords},
        ["@keyword.directive"] = colors.Blue,
        ["@keyword.exception"] = colors.Blue,
    ...
    hl.treesitter = {
        ...
        TSKeyword = {fg = c.blue, fmt = cfg.code_style.keywords},
        TSKeywordFunction = {fg = c.blue, fmt = cfg.code_style.functions},
        TSKeywordOperator =  {fg = c.blue, fmt = cfg.code_style.keywords},
        TSLabel = colors.Red,
        ...
        TSRepeat = {fg = c.blue, fmt = cfg.code_style.keywords},
...
hl.plugins.neo_tree = {
    ...
    NeoTreeSymbolicLinkTarget = colors.Blue,
}
...
hl.plugins.rainbow_delimiters = {
    ...
    RainbowDelimiterViolet = colors.Blue,
...
hl.plugins.indent_blankline = {
    ...
    IndentBlanklineIndent5 = colors.Blue,
...
hl.langs.c = {
    ...
    cStorageClass = colors.Blue,
    cTypedef = colors.Blue,
    cDefine = colors.Cyan,
    cTSInclude = colors.Blue,
    cTSConstant = colors.Cyan,
    cTSConstMacro = colors.Blue,
    cTSOperator = colors.Blue,
}

hl.langs.cpp = {
    cppStatement = { fg = c.blue, fmt = "bold" },
    cppTSInclude = colors.Blue,
    cppTSConstant = colors.Cyan,
    cppTSConstMacro = colors.Blue,
    cppTSOperator = colors.Blue,
}
...
hl.langs.php = {
    ...
    phpStructure = colors.Blue,
    phpOperator = colors.Blue,
...
hl.langs.scala = {
    ...
    scalaInterpolationBoundary = colors.Blue,
    scalaInterpolation = colors.Blue,
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('onedarkbleak.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "onedarkbleak.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.onedarkbleak_config.highlights) do
